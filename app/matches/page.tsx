      ...
      <SectionCard title="Matches">
        ...
        <div className="space-y-8">
          {MATCHES.map((match) => (
            <div key={match.round}>
              <div
                className={`overflow-hidden rounded-[24px] border bg-gradient-to-br p-1 shadow-lg print:hidden sm:rounded-[28px] ${getMatchAccent(match.course).panel} ${getMatchAccent(match.course).border}`}
              >
              ...
              {chunkPairings(match.pairings, 3).map((pairingChunk, chunkIndex) => (
                <div
                  key={`print-${match.round}-${chunkIndex}`}
                  className={`print-match-sheet hidden overflow-hidden rounded-[24px] border bg-gradient-to-br p-1 shadow-lg print:block sm:rounded-[28px] ${getMatchAccent(match.course).panel} ${getMatchAccent(match.course).border}`}
                >
                  <div className="rounded-[20px] bg-white/90 p-4 backdrop-blur-sm sm:rounded-[24px] sm:p-5 dark:bg-slate-900/85">
                                ...
                                <Scorecard
                                  ...
                                  printCompactMatchup={
                                    match.format.includes('Scramble')
                                      ? `${getPairingPlayerDetails(pairing.team1, TEAM1_PLAYERS).map((player) => player.name).join(' / ')} vs ${getPairingPlayerDetails(pairing.team2, TEAM2_PLAYERS).map((player) => player.name).join(' / ')}`
                                      : undefined
                                  }
                                  team1Players={team1Players}
                                  team2Players={team2Players}
